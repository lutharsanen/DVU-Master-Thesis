    ...
    for image in os.listdir(image_path):
        ...
        if len(resp) > 0 and type(resp) == dict:
            ...
            for face in resp:
                ...
                df = DeepFace.find(img_path = "cropped.jpg", model_name = models[3], db_path = f"{hlvu_location}/movie_knowledge_graph/{movies}/image/Person/", detector_backend = backends[4], enforce_detection = False)
                if len(df) > 0:
