...
from vision.visual_preprocessing.shotdetect.platform import STRING_TYPE
from vision.visual_preprocessing.shotdetect import frame_timecode
from vision.visual_preprocessing.shotdetect.frame_timecode import FrameTimecode
...
def validate_capture_framerate(video_names, cap_framerates, framerate=None):
    ...
    if framerate is not None:
        if isinstance(framerate, float):
            if framerate < frame_timecode.MINIMUM_FRAMES_PER_SECOND_FLOAT:
        ...
        unavailable_framerates = [(video_names[i][0], video_names[i][1]) for
                                  ...
                                  frame_timecode.MINIMUM_FRAMES_PER_SECOND_FLOAT]
...
def validate_capture_parameters(video_names, cap_frame_sizes, check_framerate=False,
    ...
    """
    bad_params = []
    max_framerate_delta = frame_timecode.MINIMUM_FRAMES_PER_SECOND_FLOAT
